...
# Use ABSOLUTE path to ensure scanner and backend write to same file
traffic_file = Path(__file__).parent.parent / "mitm_traffic.ndjson"
inject_mitm_proxy_advanced(traffic_file=str(traffic_file.resolve()))
...
**Important:** Use absolute paths! If your backend is in `backend/app/main.py`, the traffic file will be at `backend/mitm_traffic.ndjson`.

**2. Run the scanner with MITM enabled:**

```bash
# Use the SAME absolute path as in your backend
cybersec-scanner scan \
  --mitm \
  --mitm-traffic /absolute/path/to/backend/mitm_traffic.ndjson \
  --output audit_report.json
```
